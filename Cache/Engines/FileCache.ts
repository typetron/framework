...
class FileCache implements EngineInterface {
  ...
  private async read(key: string): Promise<any> {
    ...
    if (!binaryData.byteLength) return false;

    const data = JSON.parse(binaryData.toString());

    const time = data["meta"] && new Date(data["meta"]["expiresIn"]).getTime();

    if (time < new Date().getTime()) return this.deleteFIle(key);

    return JSON.stringify(data["data"]);
  }

  public async get(name: string): Promise<any> {
    if (!(await this.isCacheExist(name))) return null;
  ...
  public async set(name: string, data: any, duration: number): Promise<any> {
    if (!name) throw new Error("Cache key not provided");

    if (!data) throw new Error("Cache data not provided");

    if (duration > 0) return this.writeWithTime(name, data, duration);
  ...
  private async deleteFIle(key: string): Promise<Boolean> {
    try {
      if (!(await this.isCacheExist(key))) return false;
